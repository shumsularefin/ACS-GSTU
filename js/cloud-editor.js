...
export async function connect(config) {
  ...
  async function access(user){if(!user?.emailVerified)return '';if(user.email.toLowerCase()===SUPER_ADMIN_EMAIL)return 'super-admin';const grant=await dbSDK.getDocFromServer(dbSDK.doc(db,'chapterEditors',user.email.toLowerCase()));return grant.exists()&&grant.data().active&&['administrator','outreach'].includes(grant.data().role)?grant.data().role:'';}
  instance={
    access,
    async ensureSchema(seed){if(await access(auth.currentUser)!=='super-admin')return;await dbSDK.runTransaction(db,async tx=>{const snap=await tx.get(ref);if(snap.exists()&&snap.data().schemaVersion===2)return;const data=snap.exists()?snap.data():null;const content=data?unpackContent(data):seed;const revision=(data?.revision||0)+1;const auditId=audit(tx,'content.publish','published','Upgrade content storage for editor roles',{revision});tx.set(ref,{...packContent(content),revision,eventId:'',auditId,updatedAt:dbSDK.serverTimestamp(),updatedBy:auth.currentUser.uid});});},
    async manageEditor(email,role,active){email=editorEmail(email);if(email===SUPER_ADMIN_EMAIL)throw Error('The super-admin account cannot be changed.');if(!['administrator','outreach'].includes(role))throw Error('Choose a valid editor role.');await dbSDK.runTransaction(db,async tx=>{const r=dbSDK.doc(db,'chapterEditors',email);const auditId=audit(tx,'access.set',email,role+' / '+(active?'active':'disabled'));tx.set(r,{email,role,active,auditId,updatedAt:dbSDK.serverTimestamp(),updatedBy:auth.currentUser.uid});});},
