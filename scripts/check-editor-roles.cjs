...
(async()=>{
 ...
 for(const role of ['super-admin','administrator','outreach']){
  ...
  await page.goto('http://127.0.0.1:4173/admin.html');await page.locator('#auth-status').filter({hasText:'Role: '+role}).waitFor();if(role==='super-admin')await page.locator('#access-form').waitFor();else await page.locator('#editor-status').filter({hasText:'Ready. Live content loaded.'}).waitFor();
  assert.equal(await page.locator('#person-form').isVisible(),role==='super-admin');
  assert.ok(await page.locator('#homepage-form').isVisible());
