...
Replace paper distribution with downloadable digital certificates. The admin chooses the category first, then enters one recipient or imports Excel/CSV/JSON. Show only relevant fields: event title/date for event certificates, fiscal year/service term for executive recognition without requiring an event. Individual executive-recognition certificates must not require a spreadsheet. Support participant, champion, runner-up, second-runner-up, instructor, teacher, guest, delegate, organization/institution, executive service and general achievement categories. Executive certificates include a fiscal year/service term and optional office held.

Restore the original participant certificate with its blue ACS header and pointed QR ribbon. Use this layout for competition awards with a prominent gold/silver/bronze placement badge and no outer rectangle. Match the latest supplied formal executive SVG artwork for executive and other formal recognition. Preserve the separate GSTU logo, chemistry texture, and Advisor/Co Advisor signatures. Dr. Md. Kamruzzaman is Professor and Advisor; S. M. Fazle Rabbi is Co Advisor. Apply gold/silver/bronze placement accents. Generate the recipient name, event or service details, issue date, credential ID and scannable verification QR individually on every certificate.
...
Support automatic uploaded-image references with visible previews. Hide executive titles on founding cards and include the eight founding officers in the 2024 executive committee with their titles. Remove placeholder committee notices. Dashboard sections should collapse individually and have a Back to top control. Certificate JSON backup downloads are optional; show a shareable credential link after issuance, including for recipients without membership IDs.
